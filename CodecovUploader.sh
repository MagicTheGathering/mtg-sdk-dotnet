...
then
  ...
  profdata_files=$(find "$ddp" -name '*.profdata')
  if [ "$profdata_files" != "" ];
  then
    # xcode via profdata
    if [ "$xp" = "" ];
...
then
  files=$(eval "find $search_in -type f \( ${include_cov:5} \)$exclude_cov 2>/dev/null")
fi
...
then
  ...
  then
    ...
    then
      ...
      dotcoverage=$(find "$git_root" -name '.coverage' -or -name '.coverage.*' | head -1)
      if [ "$dotcoverage" != "" ];
      then
